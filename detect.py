...
from models.model.experimental import attempt_load
...
def detect(save_img=False):
    out, source, weights, deep_sort_weights, view_img, save_txt, imgsz, cfg, names = \
        opt.output, opt.source, opt.weights, opt.deep_sort_weights, opt.view_img, opt.save_txt, opt.img_size, opt.cfg, opt.names
    webcam = source == '0' or source.startswith('rtsp') or source.startswith('http') or source.endswith('.txt')
  
    # initialize deepsort
    deepsort_cfg = get_config()
    deepsort_cfg.merge_from_file(opt.config_deepsort)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--deep_sort_weights', type=str, default='deepsort/deep_sort/deep/checkpoint/ckpt.t7', help='ckpt.t7 path')
